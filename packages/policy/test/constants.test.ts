...
import {
  SCOPE_STATEMENT, deferralRule, callbackRule, honestIfAsked, onBehalfIntro, selfIdentity, selfGrounding,
  alwaysDeferRule
...
describe("guardrail prose constants (verbatim from current core)", () => {
  ...
  it("callbackRule pins the number for bookings", () => {
    expect(callbackRule("Alex Rivera", "+15551234567")).toContain("give them this number and only this number: +15551234567");
  });
  ...
  it("honestIfAsked is the overridable disclosure floor", () => {
    expect(honestIfAsked("Alex Rivera")).toContain("answer honestly that you are Alex Rivera's AI assistant");
  });
